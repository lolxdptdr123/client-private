...
void FontRenderer::DrawString(const char* str, float x, float y, char dropShadow, int color, JNIEnv* env)
{
	...
	if (drawStringMethod) {
		...
		const auto drawStringF = fontRendererClazz->GetMethod(env, Mapper::Get("drawString").data(), "(Ljava/lang/String;FFIZ)I");
		if (drawStringF)
			drawStringF->CallVoidMethod(env, (jobject)this, false, jstringText, x, y, color, dropShadow);
	}
