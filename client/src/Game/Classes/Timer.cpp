...
float Timer::GetTicksPerSecond(JNIEnv* env)
{
	if (this == NULL || !env)
		return 20.0f;
	...
	if (!timerClazz)
		return 20.0f;
	const auto f = timerClazz->GetField(env, Mapper::Get("ticksPerSecond").data(), "F");
	env->DeleteLocalRef((jclass)timerClazz);
	if (!f) return 20.0f;
	return f->GetFloatField(env, this);
}

void Timer::SetTicksPerSecond(float v, JNIEnv* env)
{
	if (this == NULL || !env)
		return;
